...
var data = $.jStorage.get('data') || new Object();	
...
function analyzeXml(xml, errorHandler){
	...
	for(var i=0;i < childNodes.length; i++){
		...
		if(childNodes[i].childNodes.length>0){
			...
			//$.jStorage.set(tempName, tempValue);
	...
	if(tableNodes.length>=0){
		for(var j=0;j<tableNodes.length;j++){ //row in table
			...
			for (var k=0; k < cells.length; k++){
				...
				//$.jStorage.set(cellName, cellValue);
...
function getCookiesAsQueryString(){
	...
	return $.map(data,function(value,key){return key+"="+value}).join("&")
...
{
	var expires = "";
	if (exdays==undefined || exdays=="" || exdays==null)
	{
		expires = "";
	}
	else
...
/*
function addToCookie(key, value, expires){
...
function setCookieValuesInFields() {
	...
	$.each(data,function(k,v){
        ...
        if (name!=null && name!="" && $("[id='"+name+"']") != null ){
